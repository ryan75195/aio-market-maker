"""
Fine-tune a Qwen3 model as an evaluator for ONNX cross-encoder decisions.
...
Usage:
    py -3.12 -u train.py                                          # 8B default
    py -3.12 -u train.py --base-model Qwen/Qwen3-0.6B --version v2  # 0.6B
    py -3.12 -u train.py --dry-run
    py -3.12 -u train.py --epochs 1
"""
...
try:
    ...
    USE_UNSLOTH = True
    print("Using Unsloth for fine-tuning", flush=True)
except ImportError:
    from transformers import AutoModelForCausalLM, AutoTokenizer
    from peft import LoraConfig, get_peft_model, prepare_model_for_kbit_training
    USE_UNSLOTH = False
    print("Unsloth not available, using vanilla PEFT", flush=True)
...
def parse_args():
    ...
    parser.add_argument("--base-model", type=str, default="Qwen/Qwen3-8B",
                        help="HuggingFace model ID (e.g. Qwen/Qwen3-8B, Qwen/Qwen3-0.6B)")
    parser.add_argument("--version", type=str, default="v1",
                        help="Version tag for output dirs (v1, v2, etc.)")
...
def load_audit_data(test_size=0.1, seed=42):
    ...
    print(f"Loaded {len(rows)} audited pairs (excluding errors)", flush=True)
    ...
    print(f"Test set ({len(test_rows)} pairs) saved to {test_path}", flush=True)
    ...
    print(f"Train set ({len(train_rows)} pairs) saved to {train_path}", flush=True)
...
def main():
    ...
    output_dir = Path(f"E:/Dev/ml-training/evaluator/{version}/lora_adapter")
    merged_dir = Path(f"E:/Dev/ml-training/evaluator/{version}/merged")
    ...
    print(f"\nTrain distribution: {dict(train_verdicts)}", flush=True)
    test_verdicts = Counter(r["verdict"] for r in test_rows)
    print(f"Test distribution:  {dict(test_verdicts)}", flush=True)

    if args.dry_run:
        print("\n[DRY RUN] Would load model and train. Exiting.", flush=True)
        return
    ...
    print(f"\nLoRA adapter saved to {output_dir}", flush=True)
