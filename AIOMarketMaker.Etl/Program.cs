...
{
    var limit = GetIntArg(args, "--limit");
    await BackfillPendingDescriptions(host, limit);
    return;
}
...
static async Task BackfillClassifierConfidence(IHost host, string[] args)
{
    ...
    var startFromId = GetIntArg(args, "--start-from-id") ?? 0;
...
static async Task FlushPendingUpdates(IDbContextFactory<EtlDbContext> dbFactory, List<(int Id, float Confidence, bool IsComparable)> updates)
{
...
static async Task BackfillPendingDescriptions(IHost host, int? limit)
{
    using var scope = host.Services.CreateScope();
    var db = scope.ServiceProvider.GetRequiredService<EtlDbContext>();
    var urlBuilder = scope.ServiceProvider.GetRequiredService<IEbayUrlBuilder>();
    ...
    await foreach (var result in channel.Reader.ReadAllAsync())
    {
        ...
        else
        {
            ...
            var embeddingText = BuildEmbeddingText(result.Title, result.Description);
            if (!string.IsNullOrWhiteSpace(embeddingText))
            {
                toEmbed.Add((result.ListingId, TruncateText(embeddingText, 6_000)));
            }
